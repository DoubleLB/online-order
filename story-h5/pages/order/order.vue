<template>
	<view class="order">
		...
		<view style="margin:20rpx 0rpx 50rpx;padding-bottom:10rpx;border-bottom:1px solid #f5f5f5;" 
			...
			<text class="cancelOrder" v-if="order.order_status == 1" @click="open(order.order_id)">取消订单</text>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			toDelete(){
				...
				this.$u.post('/user/order/delete',{order_id:this.orderDet})
				this.$u.toast('取消订单成功')
				//跳转后刷新页面
